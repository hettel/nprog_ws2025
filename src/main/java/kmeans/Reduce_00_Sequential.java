...
public class Reduce_00_Sequential
{
   static void reduce(Image image, final int maxCluster)
   {
      // random assignment of color pixels to a cluster
      image.pixels.stream().forEach(p -> p.centroidId = ThreadLocalRandom.current().nextInt(maxCluster));
      ...
      while (true)
      {
         ...
         Map<Integer, List<Pixel>> clusterMapList = image.pixels.stream()
